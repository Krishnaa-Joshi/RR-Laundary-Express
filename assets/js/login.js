/* =========================================
   FRONTEND MOCK AUTH FLOW
   Email/Password → OTP → Loader → Success
========================================= */

document.addEventListener("DOMContentLoaded", () => {
  const authForm = document.getElementById("authForm");
  const otpForm = document.getElementById("otpForm");
  const loader = document.getElementById("authLoader");
  const verifyBtn = document.getElementById("verifyOtpBtn");
  const otpInputs = document.querySelectorAll(".otp-inputs input");

  // SAFETY CHECK (prevents JS errors if IDs change)
  if (!authForm || !otpForm || !loader || !verifyBtn) return;

  /* -----------------------------------------
     STEP 1: Email/Password → OTP
  ----------------------------------------- */
  authForm.addEventListener("submit", (e) => {
    e.preventDefault();

    // UI switch
    authForm.classList.add("d-none");
    otpForm.classList.remove("d-none");

    // Focus first OTP input
    if (otpInputs.length) {
      otpInputs[0].focus();
    }
  });

  /* -----------------------------------------
     STEP 2: OTP → Loader → Success
  ----------------------------------------- */
  verifyBtn.addEventListener("click", () => {
    otpForm.classList.add("d-none");
    loader.classList.remove("d-none");

    // Simulated authentication delay
    setTimeout(() => {
      loader.innerHTML = `
        <h4 style="color:#22c55e; font-weight:700;">✔ Verified</h4>
        <p>Redirecting to dashboard…</p>
      `;

      // Simulated redirect
      setTimeout(() => {
        console.log("User authenticated (mock)");

        // Enable when dashboard is ready
        // window.location.href = "dashboard.html";

      }, 1500);
    }, 2000);
  });

  /* -----------------------------------------
     OTP AUTO-FOCUS + BACKSPACE SUPPORT
  ----------------------------------------- */
  otpInputs.forEach((input, index) => {
    input.addEventListener("input", () => {
      if (input.value && otpInputs[index + 1]) {
        otpInputs[index + 1].focus();
      }
    });

    input.addEventListener("keydown", (e) => {
      if (e.key === "Backspace" && !input.value && otpInputs[index - 1]) {
        otpInputs[index - 1].focus();
      }
    });
  });
});
